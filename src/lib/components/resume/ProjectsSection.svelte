<script lang="ts">
	...
	let { projects }: ProjectsSectionProps = $props();
</script>
			...
			<CardHeader class="pb-4">
				<div class="flex flex-col md:flex-row md:justify-between md:items-start gap-3">
					<div class="space-y-2">
						...
						{#if project.role}
							<p class="text-sm text-muted-foreground font-medium">{project.role}</p>
						{/if}
					</div>
					<div class="flex flex-col gap-3 text-sm md:items-end">
						{#if project.startDate}
							<div class="flex items-center gap-1 text-muted-foreground">
								...
								{project.startDate}{#if project.endDate} - {project.endDate}{/if}
							</div>
						{/if}
